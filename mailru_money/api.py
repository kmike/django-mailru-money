...
def signature(secret_key, params, hash_secret=True, encoding=ENCODING):
    """
    ...
    if hash_secret:
        key_bytes = secret_key.encode(encoding)
        secret_hash = hashlib.sha1(key_bytes).hexdigest().encode('ascii')
    else:
        secret_hash = secret_key.encode('ascii')
    values = [params[key].encode(encoding) for key in sorted_keys] + [secret_hash]
